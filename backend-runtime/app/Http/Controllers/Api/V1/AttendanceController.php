...
class AttendanceController extends Controller
{
    ...
    public function checkIn(Request $request): JsonResponse
    {
        $request->validate([
            ...
            'photo_url' => 'nullable|string|max:5000000', // Max 5MB base64
        ]);
        ...
        // Save photo if provided with validation
        $photoPath = null;
        if ($request->photo_url) {
            // Validate base64 format
            if (!preg_match('#^data:image/(jpeg|jpg|png|gif|webp);base64,#i', $request->photo_url)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid photo format. Only JPEG, PNG, GIF, and WebP are allowed.',
                ], 400);
            }
            ...
            if (!$photoData) {
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to decode photo data.',
                ], 400);
            }
            ...
            if (!$this->isValidImage($photoData)) {
                return response()->json([
                    'success' => false,
