...
class ApprovalController extends Controller
{
    ...
    {
        $query = Approval::with(['approvable.project', 'approvable.user', 'approver'])
    ...
    public function returnRevision(Request $request, Approval $approval): JsonResponse
    {
        ...
        $approvable = $approval->approvable;
        if ($approvable && method_exists($approvable, 'returnRevision')) {
            $approvable->returnRevision(auth()->id(), $request->reason);
        }
        ...
        \App\Models\AuditLog::create([
            ...
            'user_agent' => $request->userAgent(),
            'old_values' => ['status' => $oldStatus],
            'new_values' => ['status' => 'revision', 'reason' => $request->reason],
        ]);
